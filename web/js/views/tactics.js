...
import { TACTICAL_STYLES } from '../formations.js';
  ...
  for (const key in defaultTactic) {
    if (key === 'name' || key === 'description' || key === 'longDescription' || key === 'strengths' || key === 'weaknesses') {
        fields += `
            ...
            <input type="text" id="tactic-${key}" value="${selectedTactic[key]}">
        `;
    } else {
        fields += `
            <label for="tactic-${key}">${key}</label>
            <input type="range" id="tactic-${key}" min="0.5" max="1.5" step="0.01" value="${selectedTactic[key]}">
            <span>${selectedTactic[key]}</span>
        `;
    }
  }
    ...
    <div id="tactics-view">
      ...
      <div id="tactics-container">
        <div id="tactic-list-container">
          ...
          <button id="new-tactic">New Tactic</button>
        </div>
        <div id="tactic-form-container">
          <h2>Tactic Editor</h2>
          <div id="tactic-form-fields"></div>
          <button id="save-tactic">Save Tactic</button>
        </div>
      </div>
    </div>
  `;
